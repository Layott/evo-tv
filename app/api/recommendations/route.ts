...
export async function GET(req: NextRequest) {
  ...
  if (!user) {
    ...
    return NextResponse.json({ items: stripVodPlaybackAll(items, viewer), source: "trending" as const });
  }

  const items = await recommendForUser(user.id, limit);
  if (items.length === 0) {
    const fallback = await trendingVods(limit);
    return NextResponse.json({ items: stripVodPlaybackAll(fallback, viewer), source: "trending" as const });
  }
  return NextResponse.json({ items: stripVodPlaybackAll(items, viewer), source: "personalized" as const });
}
