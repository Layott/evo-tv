...
function TeamCard({ team }: { team: Team }) {
  ...
  return (
    <Link
      href={`/team/${team.slug}`}
      className="group flex items-center gap-3 rounded-xl border border-neutral-800 bg-neutral-900/60 p-3 transition-colors hover:border-neutral-700"
    >
      <img
        src={team.logoUrl}
        alt={team.name}
        className="h-14 w-14 shrink-0 rounded-md border border-neutral-800 object-cover"
      />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="truncate text-sm font-semibold">{team.name}</p>
          <span className="rounded bg-neutral-800 px-1.5 py-0.5 text-[10px] font-semibold tracking-wider text-sky-300">
            {team.tag}
          </span>
        </div>
